...
export default function Presupuestos() {
  ...
  const [modalConfirmar, setModalConfirmar] = useState(null); // presupuesto a confirmar
  ...
  // Validación reactiva de cantidad
  ...
  const guardar = async () => {
    ...
    try {
      const body   = {
        dni_cliente: dniCliente || null, observaciones: observaciones.trim() || null,
    ...
    <div>
      ...
      {presupuestos.length === 0 ? <p className="estado-carga">No hay presupuestos registrados.</p> : (
        <div className="tabla-wrapper">
          <table>
            <thead><tr><th>#</th><th>Fecha</th><th>Vence</th><th>Cliente</th><th style={{ textAlign: 'right' }}>Total</th><th>Estado</th><th></th></tr></thead>
            <tbody>
              {presupuestos.map(p => {
